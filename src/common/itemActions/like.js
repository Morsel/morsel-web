...
.directive('mrslItemLike', function(ApiItems, AfterLogin, $location, Auth, $q, $modal, $rootScope, $window, USER_LIST_NUMBER){
  return {
    ...
    template: '<div><i ng-click="toggleItemLike()" ng-class="{\'common-like-filled\': item.liked, \'common-like-empty\' : !item.liked}"></i><a ng-click="openLikes()">{{item.like_count}}<span> like{{item.like_count===1?\'\':\'s\'}}</span></a></div>'
  };
});