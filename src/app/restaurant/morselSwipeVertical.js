...
angular.module('Morsel.morselSwipeVertical', [
  'ngTouch'
])

.directive('morselSwipeVertical', ['swipe', '$window', '$document', '$parse', '$compile', function($swipe, $window, $document, $parse, $compile) {
  ...
  return {
    ...
    compile: function(tElement, tAttributes) {
      ...
      return function(scope, iElement, iAttributes, containerCtrl) {
        ...
        iAttributes.$observe('morselSwipeVertical', function(newValue, oldValue) {
        ...
        function updateMorselHeight() {
          ...
          // force the morsels height to match the viewport
          scope.morselStyle.height = {
        ...
        function scroll(y) {
          ...
          if (isNaN(y)) {
            y = scope.morselSwipes[thisMorselSwipeNum].carouselIndex * containerHeight;
          }

          offset = y;
          var move = -Math.round(offset);
          carousel.find('ul')[0].style[transformProperty] = 'translate3d(0, ' + move + 'px, 0)';
        }
        ...
        function autoScroll() {
          ...
          if (amplitude) {
            ...
            if (delta > rubberTreshold || delta < -rubberTreshold) {
              ...
              goToSlide(destination / containerHeight);
            }
          }
        }
        ...
        function getAbsMoveTreshold() {
          ...
          return moveTreshold * containerHeight;
        }
        ...
        function capPosition(y) {
          ...
          if (scope.morselSwipes[thisMorselSwipeNum].carouselIndex===0) {
            ...
            position = Math.min(((slidesCount-1)*containerHeight + getAbsMoveTreshold()), position);
          }
          return position;
        }
        ...
        function swipeEnd(coords, event, forceAnimation) {
          ...
          var minMove = getAbsMoveTreshold(),
              currentOffset = (scope.morselSwipes[thisMorselSwipeNum].carouselIndex * containerHeight),
              absMove = currentOffset - destination,
              slidesMove = -Math[absMove>=0?'ceil':'floor'](absMove / containerHeight),
